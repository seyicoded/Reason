...
const rejectLikeRequestController = async (req, res)=>{
    try{
        const user_id = (await req.user).u_id;
        const {ll_id} = req.body

        const r = await db.promise().query("UPDATE like_linker SET status = ? WHERE ll_id = ?", [2, _dt.ll_id])[0]
